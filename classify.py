...
def classify_video(video_path, frames, video_name, class_names, model, opt):
    ...
    data = Video(video_path=video_path, frames=frames,
    ...
    for i in range(video_outputs.size(0)):
        ...
        if opt.mode == 'score':
            ...
            ret.append(video_outputs[i].cpu().detach().numpy())

        results['clips'].append(clip_results)

    return results if opt.mode == 'score' else ret
